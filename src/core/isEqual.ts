...
const SymbolProto = typeof Symbol !== 'undefined' ? Symbol.prototype : null,
    ...
    hasEnumBug = !{ toString: null }.propertyIsEnumerable.call('toString', null),
    nonEnumerableProps = ['valueOf', 'isPrototypeOf', 'toString',
        'propertyIsEnumerable', 'hasOwnProperty', 'toLocaleString'];

function collectNonEnumProps(obj: any, keys: any[]) {
    ...
    if (has(obj, prop) && !keys.includes(prop)) { keys.push(prop); }
...
function keyList(obj: any): string[] {
    if (!isObject(obj)) { return []; }
    if (nativeKeys) { return nativeKeys(obj); }
    const keysArr = [];
    for (const key in obj) { if (has(obj, key)) { keysArr.push(key); } }
    // Ahem, IE < 9.
    if (hasEnumBug) { collectNonEnumProps(obj, keysArr); }
    return keysArr;
}

function eq(a: any, b: any, aStack?: unknown, bStack?: unknown): boolean {
...
function deepEq(a: any, b: any, aStack: any, bStack: any) {
    ...
    if (areArrays) {
        ...
        if (keyList(b).length !== length) { return false; }
        while (length--) {
