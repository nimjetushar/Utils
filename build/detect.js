...
var detect_each = function each(obj, iterator, context) {
  if (obj == null) {
    return;
  }
...
var remove = function remove(arr, props) {
  detect_each(arr, function (obj) {
    detect_each(props, function (prop) {
      delete obj[prop];
    });
  });
...
var detect_toString = function toString(obj) {
  obj = obj || {};
...
function () {
  function Detect() {
    ...
    this.types = ['browser', 'os', 'device'];
    this.regexes = regexes || this.fetchRegexes();
    this.families = this.fetchFamilies();
  }

  _createClass(Detect, [{
    key: "fetchRegexes",
    value: function fetchRegexes() {
      var results = {};
      this.parsers.map(function (parser) {
        results[parser] = [];
      });
      return results;
    }
    ...
    value: function parse(ua) {
      ...
      var source = ua; // Browsers Parsed

      var browser_parsers = parsers('browser'); // Operating Systems Parsed

      var os_parsers = parsers('os'); // Devices Parsed

      var device_parsers = parsers('device'); // Set Browser

      var browser = find(ua, browser_parsers);

      if (check(browser)) {
        browser = {
          name: detect_toString(browser),
          version: toVersionString(browser)
        };
      } else {
        browser = {};
      } // Set OS
      ...
      var mobile_agents = {};
      this.regexes.mobile_browser_families.map(function (str) {
        mobile_agents[str] = true;
      });
      this.regexes.mobile_os_families.map(function (str) {
        mobile_agents[str] = true;
      }); // Is Spider
      ...
      device.manufacturer = browser.man || os.man || device.man || null; // Cleanup Objects

      remove([browser, os, device], ['tablet', 'man']); // Return Agent
